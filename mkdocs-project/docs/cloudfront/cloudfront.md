...
This documentation explains how to use AWS CloudFront to create private, authenticated websites.
...
* [Authenticated website using a private S3 bucket and AWS Lambda Function](#authenticated-website-using-a-private-s3-bucket-and-lambda-function)
	...
	+ [Step 2: Create Lambda@Edge Function to Authenticate User](#step-2-create-lambdaedge-function-to-authenicate-user)
...
There following are cases where CloudFront could be used, and these are discussed in the sections
of this documentation.  **Currently this documentation focuses on experients with CloudFront,
not authoritative documentation.  If experiments work, then experiments will have been proven.**
The following are questions to answer, with primary goal to provide an authenticated `https` website using
S3 bucket for content.

1. Use CloudFront to provide authentication and `https` for an S3 bucket that is otherwise private,
using a general login.
2. Use CloudFront to provide authentication and `https` for an S3 bucket that is otherwise private,
...
This **does not** use signed URLs or signed cookies but demonstrated how to use a distribution.

* See: [A Step-by-step Guide to Creating a Password Protected S3 bucket](http://kynatro.com/blog/2018/01/03/a-step-by-step-guide-to-creating-a-password-protected-s3-bucket/) - used to a large degree for creating the example in this documentation
* See: [Serving Private Content Using Amazon CloudFront and AWS Lambda@Edge](https://aws.amazon.com/blogs/networking-and-content-delivery/serving-private-content-using-amazon-cloudfront-aws-lambdaedge/) - useful but uses Nginx server for content
...
A function will be created from scratch.
...
| ========== | ===== | ======================== | **Permissions** |
| ***Execution Role*** | | `Create a new role with basic Lambda permissions` | Use the default.  This seems reasonable for this CloudFront authentication example.  An error will likely result when creating the distribution and is described below. |
...
Change the `authUser` and `authPass` to desired values.
...
If additional changes are made, save and publish again.
...
Use the AWS Console for CloudFront.  If the first time, the page will be similar to the following.
...
| ***Allowed HTTP Methods*** | | `GET, HEAD`| Use the default. Should be enough for read-only access, and can enable others if necessary. |
...
The following error was displayed preparing this documentation:
...
* Stack overflow article "cannot create aws lambda function due to some cryptic error message"
...
Use the AWS Console for Lambda to edit the function.  Under its ***Trusted relationships*** tab, press ***Edit trust relationship***.
The `Service` did not include `edgelambda.amazonaws.com` so add that similar to Stack Overflow article above and save.
...
For example, upload a simple `index.html` file.
...
See [CloudFront Distribution Configuration](#cloudfront-distribution-configuration).
